...
import com.DC.utilities.apiEngine.apiRequests.advertising.media.ScratchpadRequests;
import com.DC.utilities.apiEngine.apiServices.advertising.media.ScratchpadApiService;
...
public class ScratchpadAPIServicesTests extends BaseClass {
    private static Logger logger;

    static List<String> saleMetrics = SharedMethods.createList("clickRevenue,relatedClickRevenue,brandClickRevenue");

    private ScratchpadRequestBody ADD_SCRATCHPAD_REQUEST = ScratchpadApiService.scratchpadRequestBody("DAILY", saleMetrics,
            DateUtility.getFirstDayOfLastSevenDays(), DateUtility.getYesterday());

    private ScratchpadRequestBody ADD_SCRATCHPAD_REQUEST_LAST_WEEK = ScratchpadApiService.scratchpadRequestBody("DAILY", saleMetrics,
            DateUtility.getFirstDayOfLastWeek(), DateUtility.getLastDayOfLastWeek());

    ScratchpadAPIServicesTests() {
        logger = Logger.getLogger(ScratchpadAPIServicesTests.class);
        PropertyConfigurator.configure("log4j.properties");
    }

    private final String DAILY = "DAILY";

    @Test(groups = "NoDataProvider", description = "Media scratchpad API test for Amazon - Verifying slicer summary data")
    public void Scratchpad_Slicer_Amazon_Api_Test() throws Exception {

        SoftAssert softAssert = new SoftAssert();

        logger.info("Extracting response");
        Response response = ScratchpadRequests.scratchpadSlicer(ADD_SCRATCHPAD_REQUEST, SecurityAPI.getTokenAPI());

        logger.info("Verifying response");
        checkResponseStatus(testMethodName.get(), HttpStatus.SC_OK, response.statusCode());

        logger.info("** Deserializing the response");
        Scratchpad scratchpadResponse = ScratchpadApiService.getScratchpadResponse(response, testMethodName.get());
        ...
        logger.info("** Execution for test case (" + "C_RS_1" + ") has completed successfully");
    }

    @Test(groups = "NoDataProvider", description = "Media scratchpad slicer yoy API test for Amazon - Verifying slicer Yoy data")
    public void Scratchpad_Slicer_Yoy_Amazon_Api_Test() throws Exception {

        SoftAssert softAssert = new SoftAssert();

        logger.info("Extracting response");
        Response response = ScratchpadRequests.scratchpadSlicerYoy(ADD_SCRATCHPAD_REQUEST_LAST_WEEK, SecurityAPI.getTokenAPI());
    ...
    @Test(groups = "NoDataProvider", description = "Media scratchpad slicer yoy API test for Amazon - Verifying slicer Yoy Aggregated BU's data")
    public void Scratchpad_Slicer_Yoy_Amazon_Api_AggregatedBUs_Test() throws Exception {

        SoftAssert softAssert = new SoftAssert();

        logger.info("Extracting response");
        Response response = ScratchpadRequests.scratchpadSlicerYoyAggBUs(ADD_SCRATCHPAD_REQUEST_LAST_WEEK, SecurityAPI.getTokenAPI());
        ...
        logger.info("Verifying slicer yoy aggregated bu's data");
        for (int i = 0; i < scratchpadResponse.data.size(); i++) {
            for (int j = 0; j < scratchpadResponse.data.get(i).getSlicerYoyData().size(); j++) {
                String slicerYoyAggBUsData = scratchpadResponse.data.get(i).getSlicerYoyData().get(j);
                softAssert.assertNotNull(slicerYoyAggBUsData, "** Failure! Null value found in response body for slicer yoy aggregated bu's data");
            }
        }
